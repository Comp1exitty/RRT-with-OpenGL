...
std::unique_ptr<Config> rrtconfig;
// 绘制体数量计数
...
{
	// RRT树
	std::unique_ptr<Tree> testTree = std::make_unique<Tree>(glm::vec2(-400, -400));
	...
	plane_list.rectangle_plane_list.emplace_back(geo_rectangle_one);
	// RRT参数设置
	std::unique_ptr<std::vector<RectanglePlane::Rectangle>> obstacle_list=std::make_unique<std::vector<RectanglePlane::Rectangle>>();
	obstacle_list->emplace_back(geo_rectangle_one);
	rrtconfig = std::make_unique<Config>(*testTree, target, 20.0f, *obstacle_list, 2000, 40.0f);
	//RRTBasic rrt(rrtconfig);
	RRTInformed rrt(*rrtconfig);
	rrt_pointer = &rrt;
	// 路径起终点单独绘制
	std::unique_ptr<Sphere> geo_sphere_root = std::make_unique<Sphere>();

	std::unique_ptr<glm::vec2> root_tmp = std::make_unique<glm::vec2>(rrt_pointer->_tree->RootNode->m_position.x, rrt_pointer->_tree->RootNode->m_position.y);
	...
	plane_list.sphere_plane_list.emplace_back(*geo_sphere_root);

	root_tmp = std::make_unique<glm::vec2>(rrtconfig->gotarget);
	geo_sphere_root->m_center = *root_tmp; geo_sphere_root->m_radius = 5.0f; geo_sphere_root->m_radio = ratio;
...
void thread1(GLFWwindow* window,glm::vec2 ratio, Geometry_List& geo_list, std::vector<glm::vec2>& geo_line_list) {
	while (!glfwWindowShouldClose(window))
	{
		if (!rrt_pointer->_tree->foundPath && count < rrtconfig->m_maxcount) {
			...
			geo_line_list.emplace_back(tmp);
			...
			{
				std::unique_lock<std::mutex> lock(mtx);
				geo_list.sphere_list.emplace_back(geo_sphere_tmp1);
				geo_list.sphere_list.emplace_back(geo_sphere_tmp2);
				if (geo_line_list.size() > 1) {
					auto it = geo_line_list.end();
					geo_line_tmp.m_end = *(it - 1);
					geo_line_tmp.m_start = *(it - 2);
					geo_list.line_list.emplace_back(geo_line_tmp);
				}
			}
			count++;
		...
		else if (rrt_pointer->_tree->foundPath&&!once)
		{
			...
			geo_line_smooth.m_width = 5; geo_line_smooth.m_color = glm::vec3(1, 0, 0); geo_line_smooth.m_ratio = ratio;
			{
				...
				while (self->parent != nullptr)
				{
					...
					geo_list.line_list.emplace_back(geo_line_smooth);
					self = self->parent;
				}
			}
			//inserted = true;
...
void trans(GLFWwindow* window, Geo_plane_list& plane_list,
	...
	if (geo_list.sphere_list.size() != sphere_num) {
		...
		for (auto i = geo_list.sphere_list.rbegin();i != geo_list.sphere_list.rbegin() + n;i++) {
			plane_list.sphere_plane_list.emplace_back(*i);
		}
	...
	if (geo_list.rectangle_list.size() != rectangle_num) {		
		...
		{
			plane_list.rectangle_plane_list.emplace_back(*i);
		}
	...
	if (geo_list.line_list.size() != line_num) {		
		...
		{
			plane_list.line_plane_list.emplace_back(*i);
		}
